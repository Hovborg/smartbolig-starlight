...
![Shelly WiFi Devices](/images/guides/shelly-wifi-hero-en.png)
...
# Or select module:
# Configuration → Configure Module → (select from list)
```
...
<Steps>
1. **Keep the default setting** in Tasmota:
   ```
   SetOption19 0
   ```
   The official Tasmota integration uses Tasmota's own discovery and requires `SetOption19 0` (the default). `SetOption19 1` switches to legacy MQTT discovery and removes devices from the integration.
...
1. **Flash Tasmota Minimal first** (frees up space):
   - In Tasmota: Firmware Upgrade → Upload tasmota-minimal.bin

2. **Flash ESPHome via HTTP:**
   - Compile ESPHome firmware (.bin file)
   - In Tasmota Minimal: Firmware Upgrade → Upload your .bin
...
<CardGrid>
  <Card title="ESPHome Advanced" icon="rocket">
    ...
    [See guide →](/en/esp32/esphome-avanceret/)
  </Card>
