...
```yaml
# configuration.yaml
# Example: Template sensor in modern format
template:
  - sensor:
      - name: "Daily Consumption"
        unit_of_measurement: "kWh"
        state: "{{ states('sensor.energy') | float(0) }}"
```

<Aside type="caution">
Template sensors do **not** support `scan_interval` — they update automatically whenever their source entities change, not on a fixed interval. To reduce polling, do it on the integration that provides the source data instead.
</Aside>
...
Empty keys like `zeroconf:` or `ssdp:` in `configuration.yaml` enable those integrations — they do not disable them, and `discovery` no longer exists in Home Assistant at all. To actually save resources, remove integrations you don't use under **Settings → Devices & Services**, and uninstall HACS integrations that are just collecting dust.
...
<CardGrid>
  <Card title="Backup & Security" icon="shield">
    ...
    [See guide →](/en/home-assistant/backup-sikkerhed/)
  </Card>
