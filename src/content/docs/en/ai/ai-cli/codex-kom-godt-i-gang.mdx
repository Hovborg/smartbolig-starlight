...
OpenAI currently recommends `gpt-5.5` for most Codex tasks, and you can also just let Codex use the default model:
...
| `codex --sandbox workspace-write` | Work in the workspace with approvals for riskier actions. Use in clean Git repos. |
...
For routine work, `gpt-5.5-mini` can stretch local-message limits further than heavier models. Use `gpt-5.5` for hard debugging, architecture, and high-risk changes.
