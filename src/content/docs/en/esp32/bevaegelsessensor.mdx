...
description: Build a simple ESP32 motion sensor with a PIR sensor, then use it for automatic lighting and other Home Assistant routines.
sidebar:
  badge:
    text: Easy
    variant: success
...
- ✅ [ESP32 set up with ESPHome](/en/esp32/)
...
<Steps>
1. **Red (VCC)** → ESP32 **5V** or **VIN** pin

2. **Yellow (OUT/Signal)** → ESP32 **GPIO5**

3. **Black (GND)** → ESP32 **GND**
</Steps>
...
<Steps>
1. Click **INSTALL** → **Wirelessly** (or USB first time)

2. Go to **Home Assistant** → **Developer Tools** → **States**
...
## 🤖 Automation: Turn on Light on Motion
...
<CardGrid>
  ...
  <Card title="💡 LED Strip" icon="setting">
    ...
    [Build LED strip →](/en/esp32/led-strip/)
  </Card>
</CardGrid>
